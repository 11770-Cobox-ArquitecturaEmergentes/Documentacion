...
En esta sección se presentan los **Epics**, **User Stories**, **Technical Stories** y **Non‑Functional Stories** del proyecto, definidos a partir de las necesidades identificadas en los segmentos de gestión de operaciones logísticas y conductores de unidades de carga. Las historias cubren las funcionalidades principales del producto digital, el sitio web estático (landing page), los componentes técnicos (API, autenticación) y los atributos de calidad (disponibilidad, seguridad, rendimiento).

Cada historia sigue el patrón **«Como [persona], quiero [acción] para [beneficio concreto]»**, donde el beneficio se vincula directamente con un dolor o necesidad identificada en las entrevistas (Needfinding). Los criterios de aceptación se expresan como escenarios **Given‑When‑Then** verificables, en tiempo presente, sin referencias a detalles de interfaz de usuario, e incluyen datos concretos y umbrales medibles.

Se ha añadido trazabilidad explícita con:
- **Entrevistas**: cita textual del entrevistado (o hallazgo agregado del análisis de entrevistas) que justifica la historia.
- **To‑Be Scenario Mapping**: fase del mapa de escenario futuro (ver sección 3.1) que la historia materializa.
- **Lean UX Hipótesis**: numeración de la hipótesis definida en el Lean UX Canvas (sección 1.2.2.3) que la historia valida.

**Nota sobre los hallazgos cuantitativos:** El análisis de entrevistas (sección 2.2.3) reveló que el 100% de los gestores reportan discrepancias en kilometraje o combustible, y que el 100% de los conductores mencionan problemas de conectividad o condiciones ambientales adversas. Estas cifras respaldan la priorización de las historias relacionadas con validación automática y operación offline.

| Epic/Story ID | Título | Descripción | Criterios de Aceptación | Relacionado con (Epic ID) | Trazabilidad con entrevista (Needfinding) | To‑Be Scenario Mapping (fase) | Lean UX Hipótesis relacionada |
|---------------|--------|-------------|-------------------------|---------------------------|-------------------------------------------|-------------------------------|-------------------------------|
| EP01 | Gestión operativa de servicios | Centraliza la planificación, consulta y seguimiento de servicios logísticos para gestores y conductores. | *No aplica (Epic)* | — | — | — | — |
| EP02 | Captura y validación inteligente | Automatiza el registro y validación de información operativa a partir de evidencia visual y datos contextuales. | *No aplica (Epic)* | — | — | — | — |
| EP03 | Incidencias y trazabilidad | Permite registrar eventos operativos, anomalías y evidencias para mejorar la auditoría y resolución de conflictos. | *No aplica (Epic)* | — | — | — | — |
| EP04 | Reportes y control operativo | Proporciona reportes, alertas e indicadores basados en datos validados para la toma de decisiones. | *No aplica (Epic)* | — | — | — | — |
| EP-LP | Sitio web estático (Landing Page) | Presenta la propuesta de valor, beneficios, segmentos y mecanismos de contacto del producto para captar clientes. | *No aplica (Epic)* | — | — | — | — |
| EP-API | API RESTful e integración | Expone servicios interoperables para aplicaciones web, móvil e integraciones externas. | *No aplica (Epic)* | — | — | — | — |
| EP06 | Operación offline y sincronización | Asegura continuidad operativa en campo con conectividad limitada y sincronización posterior. | *No aplica (Epic)* | — | — | — | — |
| US-01 | Consultar servicios programados | **Como** gestor de operaciones, **quiero** consultar los servicios logísticos programados por período y estado, **para** supervisar la operación diaria sin depender de reportes manuales que me generan entre 2 y 3 horas semanales de revisión adicional. | **Escenario 1: consulta por período** <br> **Given** que existen servicios registrados entre el 01/05/2026 y el 07/05/2026 <br> **When** el gestor selecciona ese rango de fechas en el panel <br> **Then** el sistema muestra una lista paginada con unidad (placa), conductor (nombre), ruta (origen‑destino) y estado (pendiente/en proceso/completado) en menos de 3 segundos. <br><br> **Escenario 2: filtrado por estado** <br> **Given** que hay servicios en estado “en proceso” y “completado” <br> **When** el gestor aplica el filtro “en proceso” <br> **Then** el sistema muestra únicamente los servicios con ese estado, manteniendo el resto de la información. | EP01 | *“entre dos y tres horas semanales de revisión adicional”* — Ricardo Valdivia, Coordinador de Operaciones. Además, el 100% de los gestores entrevistados reportan discrepancias recurrentes. | Gestor: “Consultar servicios programados” (columna Doing) | Hipótesis 1 (reducción tiempo de registro) |
| US-02 | Asignar conductor y unidad a servicio | **Como** gestor de operaciones, **quiero** asignar un conductor y una unidad a un servicio pendiente, **para** evitar la doble asignación de recursos y los conflictos internos que actualmente se resuelven con llamadas telefónicas y sin trazabilidad. | **Escenario 1: asignación válida** <br> **Given** un servicio con ID S-123 en estado “pendiente”, el conductor “Luis Soto” disponible y la unidad “V-456” libre en el mismo horario <br> **When** el gestor selecciona ambos recursos y confirma <br> **Then** el sistema guarda la relación (servicio‑conductor‑unidad), cambia el estado del servicio a “asignado” y muestra un mensaje de éxito. <br><br> **Escenario 2: conflicto de superposición** <br> **Given** la unidad “V-456” ya está asignada a otro servicio que se solapa en horario (ej. 10:00‑12:00) <br> **When** el gestor intenta asignarla al servicio S-123 con horario 11:00‑13:00 <br> **Then** el sistema rechaza la operación, muestra “Unidad no disponible en el horario seleccionado” y conserva el servicio sin asignar. | EP01 | *“conflictos internos por falta de evidencia”* (patrón recurrente en todos los gestores). El 100% de los gestores entrevistados carece de un mecanismo formal de validación. | Gestor: “Asignar servicio a conductor/unidad” | Hipótesis 1 (eficiencia operativa) |
| US-03 | Consultar servicios asignados del día | **Como** conductor, **quiero** consultar mis servicios asignados para el día actual, **para** planificar mi jornada y saber a qué hora debo iniciar cada ruta, sin tener que esperar a que el gestor me llame o me envíe un mensaje, reduciendo la incertidumbre y los retrasos. | **Escenario 1: consulta diaria con asignaciones** <br> **Given** que el conductor “Luis Soto” tiene 3 servicios asignados para el 07/05/2026 con horarios 08:00, 12:00 y 16:00 <br> **When** accede a la aplicación móvil y pulsa “Mis servicios hoy” <br> **Then** el sistema muestra la lista con destino, horario y estado (pendiente/en proceso) ordenada cronológicamente, todo en menos de 2 segundos. <br><br> **Escenario 2: sin asignaciones** <br> **Given** que no hay servicios asignados para ese día <br> **When** el conductor consulta <br> **Then** el sistema muestra el mensaje “No tienes servicios programados para hoy. Contacta a tu gestor.” | EP01 | *“falta de planificación clara genera conflictos”* — Edilberto Poma, Conductor. El 100% de los conductores entrevistados menciona problemas de conectividad o falta de información previa. | Conductor: “Consultar servicios asignados” | Hipótesis 1 (adopción del conductor) |
| US-04 | Registrar inicio de servicio | **Como** conductor, **quiero** registrar el inicio de un servicio cuando salgo de la base, **para** dejar constancia oficial del momento de salida y que el gestor pueda auditar cualquier retraso, evitando que me descuenten tiempo por demoras que no me pertenecen. | **Escenario 1: inicio válido** <br> **Given** un servicio asignado con ID S-123 en estado “asignado” y el conductor autenticado con permiso sobre ese servicio <br> **When** el conductor pulsa “Iniciar servicio” y confirma <br> **Then** el sistema cambia el estado a “en proceso”, registra la fecha/hora (timestamp del dispositivo) y muestra “Servicio iniciado a las HH:MM”. <br><br> **Escenario 2: inicio no autorizado** <br> **Given** el conductor intenta iniciar un servicio que no está asignado a su perfil <br> **When** pulsa “Iniciar servicio” sobre ese servicio <br> **Then** el sistema rechaza la acción con el mensaje “No puedes iniciar este servicio. No está asignado a tu usuario.” | EP01 | *“me están reclamando por algo que sí hice”* — Edilberto Poma. Además, el 100% de los conductores reporta que la falta de evidencia objetiva genera conflictos. | Conductor: “Registrar inicio de servicio” | Hipótesis 1 (registro rápido) |
| US-05 | Consultar historial de servicios | **Como** gestor, **quiero** consultar el historial de servicios ejecutados en un rango de fechas, **para** revisar el desempeño operativo y detectar patrones de incumplimiento, algo que hoy me cuesta porque perdí un cliente que exigía trazabilidad formal. | **Escenario 1: consulta con resultados** <br> **Given** que existen 15 servicios completados entre el 01/04/2026 y el 30/04/2026 <br> **When** el gestor consulta ese período <br> **Then** el sistema muestra cada servicio con fecha, unidad, conductor, kilometraje final, combustible registrado, estado de validación (validado/observado) y enlace a evidencias. <br><br> **Escenario 2: período sin datos** <br> **Given** que no hay servicios en el rango solicitado (ej. futuro) <br> **When** el gestor realiza la consulta <br> **Then** el sistema muestra “No se encontraron servicios en el período seleccionado” y ofrece sugerir otro rango. | EP01 | *“perdí un contrato retail porque no pudimos entregar evidencia digital trazable”* — Miriam Ccori, Jefa de Operaciones. | Gestor: “Consultar historial de servicios” | Hipótesis 2 (confianza en datos) |
| US-06 | Capturar evidencia de odómetro | **Como** conductor, **quiero** capturar una foto del odómetro al iniciar y al finalizar cada servicio, **para** registrar el kilometraje sin anotarlo manualmente y evitar las diferencias de 50‑60 km que luego me cuestionan sin prueba. | **Escenario 1: captura válida** <br> **Given** el conductor está en el punto de control del servicio (inicio o fin) y la cámara está activa <br> **When** toma una foto donde el odómetro es legible (resolución ≥ 1024x768, enfoque nítido, iluminación ≥ 200 lux) <br> **Then** el sistema guarda la imagen localmente, la asocia al servicio, muestra “Evidencia de odómetro guardada” y registra un timestamp. <br><br> **Escenario 2: foto ilegible** <br> **Given** la foto capturada está borrosa (varianza de Laplaciano < 100) o con poca luz (lux < 50) <br> **When** el sistema analiza la imagen localmente <br> **Then** muestra el mensaje “Foto no clara, por favor toma otra” y no guarda la imagen, permitiendo hasta 3 reintentos. | EP02 | *“diferencias de 50 a 60 km sin justificación”* — Ricardo Valdivia. El 100% de los gestores reporta inconsistencias en kilometraje o combustible. | Conductor: “Capturar evidencia de odómetro” | Hipótesis 1 (reducción tiempo registro) y 3 (detección inconsistencias) |
| US-07 | Extraer kilometraje automáticamente | **Como** gestor, **quiero** que el sistema extraiga automáticamente la lectura del odómetro desde la foto usando OCR local, **para** resolver las discrepancias de kilometraje con evidencia objetiva y dejar de “adivinar” como hago hoy, reduciendo las 2‑3 horas semanales de conciliación. | **Escenario 1: extracción exitosa** <br> **Given** una foto de odómetro con el marcador claramente visible (dígitos de al menos 20 píxeles de alto, contraste ≥ 0.7) <br> **When** el sistema ejecuta el modelo de OCR en el dispositivo <br> **Then** extrae el valor numérico (ej. 12450), lo registra como “extraído automáticamente”, y marca la evidencia como “validada por sistema” en menos de 2 segundos. <br><br> **Escenario 2: valor inconsistente con el contexto** <br> **Given** el valor extraído es 12450 pero el kilometraje de inicio del servicio era 12350 (diferencia de solo 100 km en una ruta que debería ser 400 km) <br> **When** el sistema valida la coherencia <br> **Then** marca la evidencia como “inconsistente – requiere revisión del gestor” y genera una alerta de anomalía. | EP02 | *“eso no es gestión, es adivinar”* — Ricardo Valdivia. El 100% de los gestores carece de mecanismo automático de validación. | Gestor: “Validación automática de odómetro” | Hipótesis 3 (detección de inconsistencias) |
| US-08 | Validar evidencia de entrega | **Como** gestor, **quiero** que el sistema valide automáticamente la evidencia de entrega (foto de la guía firmada), **para** contar con un respaldo verificable que me permita responder reclamos de clientes, algo que hoy me ha costado contratos. | **Escenario 1: validación exitosa** <br> **Given** una foto de la guía de remisión con el texto “RECIBIDO” y una firma visible <br> **When** el sistema procesa la imagen (OCR + detección de firma) <br> **Then** la marca como “entrega validada”, extrae la fecha/hora de recepción y asocia la evidencia al servicio. <br><br> **Escenario 2: evidencia incompleta** <br> **Given** la foto no contiene firma o está borrosa <br> **When** se analiza <br> **Then** el sistema marca la entrega como “observada – requiere verificación manual” y notifica al gestor. | EP02 | *“perdí un cliente retail porque no pudimos entregar evidencia digital trazable”* — Miriam Ccori. | Gestor: “Validar evidencia de entrega” | Hipótesis 2 (confianza en información) |
| US-09 | Validar comprobante de combustible | **Como** gestor, **quiero** validar automáticamente los comprobantes de combustible (foto de la boleta), **para** detectar inconsistencias como grifos fuera de ruta o fechas incorrectas, porque ya he tenido boletas que no corresponden al servicio, especialmente en unidades alquiladas. | **Escenario 1: comprobante consistente** <br> **Given** un comprobante con fecha 07/05/2026, monto S/150, y la ruta del servicio incluye un grifo de la misma cadena en esa ubicación <br> **When** el sistema extrae los metadatos (fecha, monto, RUC del grifo) y los contrasta con el recorrido planificado <br> **Then** lo marca como “consistente” y lo asocia al servicio. <br><br> **Escenario 2: inconsistencia geográfica** <br> **Given** el comprobante es de un grifo en Chincha pero el servicio transcurrió solo en Lima Metropolitana <br> **When** se valida la ubicación (GPS de la foto o texto de dirección) <br> **Then** el sistema genera una alerta “Combustible fuera de ruta – revisar” y retiene la evidencia para auditoría. | EP02 | *“boletas de grifos que no corresponden a la ruta”* — Jorge Mamani, Gerente General. | Gestor: “Validar comprobante de combustible” | Hipótesis 3 (validación automática) |
| US-10 | Registrar captura guiada de evidencia | **Como** conductor, **quiero** recibir retroalimentación inmediata sobre la calidad de la foto (enfoque, iluminación), **para** corregir la captura en el momento y no tener que repetir el proceso más tarde, porque “quien diseña la app capaz no las piensa”. | **Escenario 1: calidad aceptable** <br> **Given** la cámara activa y el conductor enfoca el odómetro <br> **When** el sistema detecta que la imagen cumple (enfoque, contraste, región de interés) <br> **Then** muestra un ícono verde “Buena foto” y permite guardar. <br><br> **Escenario 2: baja iluminación** <br> **Given** la luz ambiental es < 100 lux <br> **When** el conductor intenta capturar <br> **Then** el sistema muestra “Poca luz, activa el flash” y activa automáticamente el flash si está disponible; si aún así no mejora, sugiere usar linterna. | EP02 | *“quien diseña la app capaz no las piensa”* — Hipólito Ccama, Conductor. El 100% de los conductores menciona condiciones adversas (luz, señal, cansancio). | Conductor: “Captura guiada” | Hipótesis 5 (calidad de imagen) |
| US-11 | Registrar datos manuales de contingencia | **Como** conductor, **quiero** ingresar manualmente el kilometraje o combustible cuando la foto no sea válida después de varios intentos, **para** no detener la operación, siempre que justifique el motivo, porque hoy los desvíos por huaycos o bloqueos no quedan documentados. | **Escenario 1: contingencia permitida** <br> **Given** el sistema ha rechazado la foto 3 veces por mala calidad <br> **When** el conductor pulsa “Ingreso manual” y escribe “Fotografía imposible por lluvia intensa” <br> **Then** el sistema guarda el dato manual con estado “pendiente de revisión” y registra la justificación. <br><br> **Escenario 2: contingencia sin justificación** <br> **Given** el conductor intenta ingresar datos manuales sin proporcionar un motivo <br> **When** pulsa “Guardar” <br> **Then** el sistema muestra “Debes justificar el ingreso manual” y no guarda. | EP02 | *“desvíos por huaycos no quedan documentados”* — Edilberto Poma. El 100% de los conductores reporta eventos no planificados sin registro formal. | Conductor: “Registro manual de contingencia” | Hipótesis 3 (manejo de excepciones) |
| US-12 | Detectar anomalías operativas | **Como** gestor, **quiero** que el sistema detecte automáticamente anomalías (kilometraje inusualmente alto/bajo, consumo fuera de rango), **para** intervenir oportunamente antes de que se conviertan en un reclamo de cliente, como ya me pasó con odómetros manipulados en unidades alquiladas. | **Escenario 1: anomalía detectada** <br> **Given** un servicio donde el kilometraje reportado (600 km) supera en más del 20% la distancia georreferenciada (500 km) <br> **When** el sistema ejecuta la validación automática al finalizar el servicio <br> **Then** genera una alerta “Posible sobrekilometraje” en el panel del gestor, con trazabilidad del caso. <br><br> **Escenario 2: sin anomalías** <br> **Given** todos los datos están dentro de los rangos esperados (±5%) <br> **When** se completa la validación <br> **Then** no se generan alertas y el servicio se marca como “validado”. | EP02 | *“casos documentados de odómetros manipulados en unidades alquiladas”* — Jorge Mamani. | Gestor: “Alertas de anomalías” | Hipótesis 3 (detección de fraudes) |
| US-13 | Reportar incidencia operativa | **Como** conductor, **quiero** reportar una incidencia (accidente, avería, bloqueo) durante el servicio, **para** dejar constancia oficial del evento y que el gestor pueda tomar acciones sin tener que llamarme, porque hoy los desvíos por huaycos no se registran y luego me descuentan. | **Escenario 1: registro exitoso** <br> **Given** ocurre un bloqueo por huayco en la ruta <br> **When** el conductor selecciona “Reportar incidencia”, elige categoría “Bloqueo vial”, escribe “Huayco en km 45, paso suspendido por 2 horas” y adjunta una foto opcional <br> **Then** el sistema guarda la incidencia con timestamp, ubicación (GPS), la asocia al servicio y envía una notificación al gestor. <br><br> **Escenario 2: datos incompletos** <br> **Given** el conductor no selecciona ninguna categoría y deja el campo descripción vacío <br> **When** pulsa “Guardar” <br> **Then** el sistema muestra “Debes completar al menos la categoría y una descripción breve”. | EP03 | *“desvíos por huaycos no quedan documentados”* — Edilberto Poma. | Conductor: “Reportar incidencia” | Hipótesis 4 (operación offline) |
| US-14 | Registrar paradas no planificadas | **Como** conductor, **quiero** registrar una parada no planificada (control policial, descanso forzoso), **para** respaldar los retrasos con evidencia de ubicación y hora, porque una retención de SUTRAN me generó un reclamo sin poder defenderme. | **Escenario 1: parada registrada** <br> **Given** el conductor se detiene en un peaje por control de SUTRAN <br> **When** pulsa “Registrar parada”, selecciona “Control policial” y confirma <br> **Then** el sistema guarda la hora de inicio, ubicación, y luego al reanudar guarda la hora de fin, calculando la duración. <br><br> **Escenario 2: sincronización posterior** <br> **Given** el conductor registra la parada sin conexión (offline) <br> **When** más tarde recupera señal <br> **Then** el sistema sincroniza automáticamente la parada con todos sus datos sin pérdida. | EP03 | *“retención de SUTRAN no quedó documentada”* — Hipólito Ccama. | Conductor: “Registrar parada no planificada” | Hipótesis 4 (sincronización offline) |
| US-15 | Consultar incidencias y observaciones | **Como** gestor, **quiero** consultar el listado de incidencias y observaciones activas, ordenadas por severidad, **para** priorizar las acciones correctivas y responder a los cuestionamientos del directorio con datos, no con intuición, evitando las 3‑4 horas diarias de conciliación manual. | **Escenario 1: consulta de incidencias activas** <br> **Given** existen 3 incidencias (1 de alta severidad, 2 de baja) y 5 observaciones <br> **When** el gestor accede al módulo “Incidencias” <br> **Then** el sistema muestra primero las de alta severidad, luego las de baja, y al final las observaciones, cada una con servicio asociado, fecha y estado. <br><br> **Escenario 2: filtrado por severidad** <br> **Given** el gestor selecciona filtro “Alta severidad” <br> **Then** se muestran solo las incidencias con esa etiqueta. | EP03 | *“entre tres y cuatro horas diarias de transcripción y conciliación”* — Miriam Ccori. | Gestor: “Consultar incidencias” | Hipótesis 2 (confianza del gestor) |
| US-16 | Resolver incidencia con trazabilidad | **Como** gestor, **quiero** cerrar una incidencia registrando la resolución y evidencia de cierre, **para** mantener un historial auditable y demostrar que se tomó acción correctiva, algo que los clientes me exigen en sus auditorías. | **Escenario 1: cierre válido** <br> **Given** una incidencia abierta con ID I-789 <br> **When** el gestor escribe “Se reprogramó la entrega, el cliente recibió el producto el día siguiente sin recargo” y adjunta un comprobante de acuerdo <br> **Then** el sistema cambia el estado a “resuelta”, almacena la resolución y la fecha. <br><br> **Escenario 2: cierre sin justificación** <br> **Given** el gestor intenta cerrar sin agregar ningún comentario ni evidencia <br> **When** pulsa “Cerrar” <br> **Then** el sistema muestra “Debes indicar la acción tomada para cerrar la incidencia”. | EP03 | *“perdí un contrato retail porque no pudimos entregar evidencia digital trazable”* — Miriam Ccori. | Gestor: “Resolver incidencia” | Hipótesis 2 (trazabilidad) |
| US-17 | Consultar trazabilidad completa del servicio | **Como** gestor, **quiero** ver una línea de tiempo completa de cada servicio: asignación, inicio, evidencias, incidencias, validaciones y cierre, **para** poder auditar la ejecución paso a paso y responder a cualquier reclamo con objetividad, porque hoy “me da vergüenza mostrarles mi cuaderno”. | **Escenario 1: trazabilidad completa** <br> **Given** un servicio que tuvo 2 evidencias, 1 incidencia y validación automática parcial <br> **When** el gestor selecciona “Ver trazabilidad” <br> **Then** el sistema muestra orden cronológico: asignación (fecha), inicio (timestamp), evidencia 1 (foto + extracción OK), incidencia (reporte y resolución), evidencia 2 (observada), cierre. <br><br> **Escenario 2: información parcial** <br> **Given** un servicio donde faltan algunas evidencias obligatorias <br> **When** se consulta la trazabilidad <br> **Then** el sistema resalta en rojo los elementos faltantes y muestra mensaje “Evidencia de odómetro final no registrada”. | EP03 | *“me da vergüenza mostrarles mi cuaderno”* — Jorge Mamani. | Gestor: “Trazabilidad completa” | Hipótesis 2 (información verificable) |
| US-18 | Visualizar alertas operativas | **Como** gestor, **quiero** ver un panel de alertas activas (anomalías, incidencias no resueltas) con niveles de severidad, **para** actuar rápidamente y dejar atrás la validación reactiva que hoy solo ocurre cuando hay un reclamo. | **Escenario 1: alertas activas** <br> **Given** hay 3 alertas: 1 de alta (combustible fuera de ruta), 2 de media (evidencia de entrega observada) <br> **When** el gestor ingresa al dashboard <br> **Then** las alertas se muestran agrupadas por severidad, con íconos de color rojo (alta) y amarillo (media), y un contador en la barra de navegación. <br><br> **Escenario 2: sin alertas** <br> **Given** no hay ninguna anomalía ni incidencia abierta <br> **When** se carga el dashboard <br> **Then** se muestra “No hay alertas activas. La operación está en estado normal.” | EP04 | *“visibilidad reactiva y parcial”* — Miriam Ccori. El 100% de los gestores valida solo cuando hay reclamo. | Gestor: “Alertas operativas” | Hipótesis 3 (detección proactiva) |
| US-19 | Consultar indicadores de confiabilidad | **Como** gestor, **quiero** consultar indicadores como % de evidencias validadas automáticamente, número de observaciones, y tiempo promedio de revisión, **para** evaluar la calidad del registro operativo y justificar mejoras ante la gerencia con datos, no con intuición. | **Escenario 1: indicadores disponibles para el mes** <br> **Given** en abril 2026 se procesaron 120 servicios <br> **When** el gestor consulta “Indicadores de confiabilidad” para el período 01/04‑30/04 <br> **Then** el sistema muestra: 78% validación automática, 22% observaciones, tiempo promedio de revisión 12 min (vs 45 min previo). <br><br> **Escenario 2: filtro temporal** <br> **Given** el gestor cambia el filtro a la última semana <br> **Then** se recalcula y muestra los indicadores para ese subperíodo. | EP04 | *“incapacidad de proyectar costos con datos confiables”* — Ricardo Valdivia. | Gestor: “Indicadores de confiabilidad” | Hipótesis 6 (valor para el negocio) |
| US-20 | Generar reporte de servicios validados | **Como** gestor, **quiero** generar un reporte en PDF o Excel de los servicios completados con sus evidencias validadas, **para** sustentar decisiones ante clientes o la gerencia, porque perdí un cliente que solicitaba reportes formales que no pude proveer. | **Escenario 1: generación exitosa** <br> **Given** se selecciona el período 01/04‑30/04 y el formato PDF <br> **When** se pulsa “Generar reporte” <br> **Then** el sistema produce un archivo PDF descargable en menos de 10 segundos, que incluye lista de servicios, kilómetros validados, número de entregas, incidencias resueltas y un resumen de indicadores. <br><br> **Escenario 2: período sin datos** <br> **Given** no hay servicios en el rango solicitado <br> **When** se intenta generar <br> **Then** el sistema muestra “No hay datos en el período seleccionado” y no genera el archivo. | EP04 | *“perdió un cliente porque no pudo entregar reportes digitales”* — Jorge Mamani. | Gestor: “Generar reporte” | Hipótesis 2 (información para auditoría) |
| US-21 | Comparar desempeño por unidad y conductor | **Como** gestor, **quiero** comparar métricas (kilometraje promedio, consumo por km, incidencias) entre diferentes unidades y conductores, **para** identificar patrones de ineficiencia, como la variabilidad del 30% en consumo que hoy no puedo explicar. | **Escenario 1: comparación disponible** <br> **Given** existen al menos 5 servicios completados para las unidades A, B y C en el último mes <br> **When** el gestor selecciona estas unidades y el indicador “consumo l/100km” <br> **Then** el sistema muestra un gráfico de barras con el consumo de cada unidad, destacando la de mayor consumo. <br><br> **Escenario 2: datos insuficientes** <br> **Given** una unidad tiene solo 1 servicio registrado <br> **When** se solicita comparación <br> **Then** el sistema muestra un aviso “Datos insuficientes para la unidad X (mínimo 3 servicios requeridos)”. | EP04 | *“variabilidad inexplicable de consumo de combustible hasta 30%”* — Miriam Ccori. | Gestor: “Comparar desempeño” | Hipótesis 6 (valor para el negocio) |
| US-22 | Consultar propuesta de valor del producto | **Como** visitante (dueño de PyME de transporte), **quiero** acceder a una landing page que explique claramente qué problema resuelve CoBox Smart Vision y sus beneficios, **para** decidir si es relevante para mi operación, porque el término “IA” me genera distancia y necesito lenguaje operativo concreto. | **Escenario 1: acceso a contenido principal** <br> **Given** el visitante ingresa a la URL del producto <br> **When** la página carga <br> **Then** muestra en la sección principal: “Audita tus servicios con evidencia visual verificable”, seguido de 3 beneficios principales (reducción de discrepancias, ahorro de tiempo, trazabilidad). <br><br> **Escenario 2: navegación por secciones** <br> **Given** el visitante hace clic en “Funcionalidades” <br> **Then** se desplaza a una sección que describe captura offline, validación automática y reportes, con íconos y texto legible en móvil. | EP-LP | *“el término IA le genera distancia; el valor debe comunicársele en lenguaje operativo concreto”* — Jorge Mamani. | *No aplica (landing page)* | Hipótesis 6 (disposición a pagar) |
| US-23 | Consultar funcionalidades clave | **Como** visitante del segmento logístico, **quiero** ver una lista clara de funcionalidades (captura de odómetro, validación IA, reportes, offline), **para** entender rápidamente cómo la solución mejora la trazabilidad y la validación operativa. | **Escenario 1: consulta de funcionalidades** <br> **Given** el visitante está en la landing page <br> **When** hace scroll a la sección “¿Qué puede hacer CoBox Smart Vision?” <br> **Then** se muestran 4 tarjetas con título, breve descripción y un ícono representativo. <br><br> **Escenario 2: contenido segmentado** <br> **Given** el visitante selecciona la pestaña “Para gestores” <br> **Then** se muestran funcionalidades específicas: dashboard, alertas, reportes. En la pestaña “Para conductores”: captura guiada, offline, incidencias. | EP-LP | Basado en la necesidad de simplicidad expresada por Jorge Mamani y Ricardo Valdivia (100% de los gestores priorizan facilidad de uso). | *No aplica* | Hipótesis 6 (adopción) |
| US-24 | Solicitar demostración comercial | **Como** visitante interesado, **quiero** completar un formulario corto (nombre, empresa, correo, teléfono) para solicitar una demostración personalizada, **para** ver la solución aplicada a mi flota real sin compromiso, porque exijo verla funcionar en una unidad real antes de pagar. | **Escenario 1: solicitud exitosa** <br> **Given** el visitante llena todos los campos obligatorios con formato válido (email válido, teléfono 9 dígitos) <br> **When** pulsa “Solicitar demo” <br> **Then** el sistema muestra “Gracias, un asesor se contactará en las próximas 24 horas” y envía una notificación al equipo comercial. <br><br> **Escenario 2: datos inválidos** <br> **Given** el visitante omite el teléfono o escribe un email sin @ <br> **When** envía el formulario <br> **Then** el sistema resalta los campos incorrectos y muestra “Corrige los errores antes de enviar”. | EP-LP | *“exige verla funcionar en una de sus unidades reales antes de comprometerse”* — Jorge Mamani. | *No aplica* | Hipótesis 6 (disposición a pagar) |
| US-25 | Consultar evidencia de resultados y casos de uso | **Como** visitante, **quiero** leer casos de uso reales o pilotos con métricas como reducción de discrepancias o ahorro de tiempo, **para** confiar en que la solución ya ha sido validada en condiciones similares a las mías. | **Escenario 1: revisión de casos de uso** <br> **Given** el visitante accede a la sección “Resultados” <br> **When** hace clic en “Ver caso de estudio – Transportes Valdivia” (piloto simulado) <br> **Then** se muestra un resumen: “Reducción del 35% en tiempo de revisión de discrepancias, 0 pérdida de evidencias”. <br><br> **Escenario 2: navegación temática** <br> **Given** el visitante selecciona el filtro “PyME de distribución urbana” <br> **Then** se muestran solo los casos aplicables a ese segmento. | EP-LP | Basado en la necesidad de validación empírica de todos los entrevistados (Ricardo, Miriam, Jorge). El 100% de los gestores requiere resultados medibles. | *No aplica* | Hipótesis 6 (confianza en el producto) |
| US-26 | Contactar al equipo comercial | **Como** visitante con dudas específicas (precios, integración), **quiero** enviar un mensaje a través de un formulario de contacto, **para** obtener respuestas directas sin pedir una demostración completa. | **Escenario 1: envío exitoso** <br> **Given** el visitante completa nombre, email y mensaje (mínimo 10 caracteres) <br> **When** pulsa “Enviar consulta” <br> **Then** el sistema registra la consulta y responde con un correo automático de confirmación. <br><br> **Escenario 2: validación de campos** <br> **Given** el visitante deja el mensaje vacío <br> **When** intenta enviar <br> **Then** el sistema muestra “El mensaje no puede estar vacío”. | EP-LP | Basado en la necesidad de acompañamiento expresada por Jorge Mamani y Miriam Ccori (priorizan soporte postventa real). | *No aplica* | Hipótesis 6 (soporte) |
| TS-01 | Autenticación y autorización API | **Como** Developer, **quiero** implementar autenticación OAuth2 (Auth0) con tokens JWT y control de acceso basado en roles, **para** proteger los endpoints de la API y garantizar que cada usuario solo acceda a los recursos autorizados, cumpliendo con requisitos de seguridad. | **Escenario 1: autenticación válida** <br> **Given** credenciales válidas de un gestor <br> **When** se llama a `/api/auth/login` <br> **Then** el sistema retorna un token JWT con expiración de 1 hora y claims `role=gestor`. <br><br> **Escenario 2: acceso denegado por rol** <br> **Given** un token de conductor <br> **When** se intenta acceder a `/api/reports/delete` <br> **Then** la API retorna HTTP 403 Forbidden con mensaje “Permiso insuficiente”. | EP-API | — | *No aplica* | — |
| TS-02 | Gestión de servicios vía API | **Como** Developer, **quiero** disponer de endpoints RESTful para crear, consultar y actualizar servicios (CRUD), **para** que la aplicación web y móvil puedan consumirlos de forma estandarizada, y futuros ERP se puedan integrar. | **Escenario 1: consulta de servicios** <br> **Given** token válido de gestor <br> **When** `GET /api/services?date=2026-05-07` <br> **Then** retorna lista de servicios en JSON con estructura definida (OpenAPI). <br><br> **Escenario 2: creación con datos inválidos** <br> **Given** se envía un servicio sin fecha de inicio <br> **When** `POST /api/services` <br> **Then** retorna HTTP 400 y un array de errores de validación. | EP-API | — | *No aplica* | — |
| TS-03 | Captura y validación de evidencias vía API | **Como** Developer, **quiero** un endpoint para subir imágenes de evidencias y otro para consultar el estado de validación, **para** que la app móvil pueda enviar fotos y verificar si fueron procesadas correctamente. | **Escenario 1: carga exitosa de evidencia** <br> **Given** un archivo JPEG < 5 MB y el metadato `serviceId` <br> **When** `POST /api/evidences` multipart <br> **Then** retorna HTTP 202 Accepted con un `evidenceId` y un mensaje “En cola de procesamiento”. <br><br> **Escenario 2: evidencia no válida** <br> **Given** el archivo no es una imagen (ej. PDF) <br> **When** se intenta subir <br> **Then** retorna HTTP 415 Unsupported Media Type. | EP-API | — | *No aplica* | — |
| TS-04 | Gestión de incidencias y alertas vía API | **Como** Developer, **quiero** endpoints para reportar incidencias, consultar alertas y cerrar casos, **para** integrar la lógica de incidencias con otros sistemas de monitoreo que el cliente pueda tener. | **Escenario 1: registro de incidencia** <br> **Given** un conductor autenticado <br> **When** `POST /api/incidents` con categoría, descripción y `serviceId` <br> **Then** retorna HTTP 201 Created y el ID de la incidencia. <br><br> **Escenario 2: consulta de alertas activas** <br> **Given** gestor autenticado <br> **When** `GET /api/alerts?status=active` <br> **Then** retorna lista de alertas con severidad y servicio asociado. | EP-API | — | *No aplica* | — |
| TS-05 | Reportes e indicadores vía API | **Como** Developer, **quiero** endpoints que entreguen métricas agregadas (porcentaje de validación, tiempo promedio de revisión) y generen reportes en PDF, **para** que el frontend pueda mostrar dashboards sin lógica de negocio embebida. | **Escenario 1: consulta de indicadores** <br> **Given** token de gestor y filtros de fecha <br> **When** `GET /api/metrics/reliability?from=2026-04-01&to=2026-04-30` <br> **Then** retorna JSON con `autoValidationRate`, `manualEntries`, `avgReviewTimeMinutes`. <br><br> **Escenario 2: generación de reporte** <br> **Given** los mismos filtros <br> **When** `POST /api/reports/generate` con formato=pdf <br> **Then** retorna un enlace de descarga temporal (URL firmada) válida por 10 minutos. | EP-API | — | *No aplica* | — |
| NF-01 | Recuperación ante fallos de red | **Como** conductor, **quiero** que si la sincronización falla, el sistema reintente automáticamente con backoff exponencial (1s, 2s, 4s hasta 3 veces), **para** no perder datos en rutas con cobertura inestable, como los tramos sin señal entre Cañete, Chincha y Nasca. | **Escenario 1: reintento exitoso** <br> **Given** hay 5 evidencias pendientes y la conexión se restablece después de 2 fallos <br> **When** el sistema ejecuta la sincronización automática <br> **Then** después del segundo reintento, envía todos los datos y los marca como sincronizados, sin intervención del usuario. <br><br> **Escenario 2: fallo definitivo** <br> **Given** después de 3 reintentos la conexión sigue caída <br> **Then** el sistema mantiene los datos en estado “pendiente” y programa un nuevo intento en 30 minutos, notificando al conductor con un ícono de advertencia. | EP06 | *“falta de señal en la ruta al sur”* — Hipólito Ccama. El 100% de los conductores menciona problemas de conectividad. | EP06 (operación offline) | Hipótesis 4 (sincronización offline) |
| NF-02 | Seguridad de evidencias en reposo | **Como** administrador de plataforma, **quiero** que todas las evidencias almacenadas en el dispositivo móvil estén cifradas con AES-256, **para** proteger datos sensibles (fotos de documentos, geolocalización) en caso de pérdida o robo del teléfono, cumpliendo con la normativa de protección de datos. | **Escenario 1: cifrado en almacenamiento local** <br> **Given** el conductor captura una evidencia sin conexión <br> **When** la imagen se guarda en el disco local <br> **Then** el archivo se escribe cifrado usando una clave derivada de un secreto seguro (no hardcodeado) y solo la aplicación puede descifrarlo. <br><br> **Escenario 2: acceso sin autorización** <br> **Given** un tercero extrae la tarjeta SD del dispositivo <br> **When** intenta leer los archivos de imágenes <br> **Then** no puede descifrar el contenido sin la clave de la aplicación. | EP06 | Basado en la preocupación de privacidad expresada por Ricardo Valdivia y las restricciones del proyecto (C-8). | EP06 (seguridad) | — |
...
Siguiendo los lineamientos del curso y las mejores prácticas ágiles (WSJF simplificado), se ha priorizado:
1. **Landing page (US-22 a US-26):** Porque sin clientes interesados no hay proyecto. El sitio web estático es la primera interacción con el mercado y debe estar listo desde el Sprint 1.
2. **Funcionalidades básicas para conductores (US-03, US-04, US-06, US-10, US-27, US-28, US-29):** Son el corazón de la captura de datos. Sin operación offline y captura confiable, el producto no funciona en campo.
3. **Validación automática y alertas para gestores (US-07, US-08, US-09, US-12, US-18, etc.):** Aportan el mayor valor de negocio (reducción de discrepancias y tiempo de revisión), pero dependen de que los conductores capturen bien las evidencias.
4. **Reportes y trazabilidad (US-19, US-20, US-21, US-17, etc.):** Útiles para la toma de decisiones y auditoría, pero no bloquean la operación diaria.
5. **Historias técnicas (TS-01 a TS-05 y NF-01, NF-02):** Necesarias para la integración, seguridad y robustez, pero se pueden desarrollar en paralelo o después de las funcionalidades core.

| # Orden | User Story ID | Título | Descripción | Story Points (1/2/3/5/8) |
|---------|---------------|--------|-------------|--------------------------|
| 1 | US-22 | Consultar propuesta de valor del producto | Como visitante (dueño de PyME de transporte), quiero acceder a una landing page que explique claramente qué problema resuelve CoBox Smart Vision y sus beneficios, para decidir si es relevante para mi operación. | 3 |
| 2 | US-24 | Solicitar demostración comercial | Como visitante interesado, quiero completar un formulario corto (nombre, empresa, correo, teléfono) para solicitar una demostración personalizada, para ver la solución aplicada a mi flota real sin compromiso. | 5 |
| 3 | US-23 | Consultar funcionalidades clave | Como visitante del segmento logístico, quiero ver una lista clara de funcionalidades (captura de odómetro, validación IA, reportes, offline), para entender rápidamente cómo la solución mejora la trazabilidad. | 3 |
| 4 | US-26 | Contactar al equipo comercial | Como visitante con dudas específicas (precios, integración), quiero enviar un mensaje a través de un formulario de contacto, para obtener respuestas directas sin pedir una demostración completa. | 2 |
| 5 | US-25 | Consultar evidencia de resultados y casos de uso | Como visitante, quiero leer casos de uso reales o pilotos con métricas como reducción de discrepancias o ahorro de tiempo, para confiar en que la solución ya ha sido validada. | 3 |
| 6 | US-03 | Consultar servicios asignados del día | Como conductor, quiero consultar mis servicios asignados para el día actual, para planificar mi jornada y saber a qué hora debo iniciar cada ruta, sin tener que esperar a que el gestor me llame. | 5 |
| 7 | US-04 | Registrar inicio de servicio | Como conductor, quiero registrar el inicio de un servicio cuando salgo de la base, para dejar constancia oficial del momento de salida y que el gestor pueda auditar cualquier retraso. | 3 |
| 8 | US-06 | Capturar evidencia de odómetro | Como conductor, quiero capturar una foto del odómetro al iniciar y al finalizar cada servicio, para registrar el kilometraje sin anotarlo manualmente y evitar errores de transcripción. | 5 |
| 9 | US-10 | Registrar captura guiada de evidencia | Como conductor, quiero recibir retroalimentación inmediata sobre la calidad de la foto (enfoque, iluminación), para corregir la captura en el momento y no tener que repetir el proceso más tarde. | 3 |
| 10 | US-27 | Registrar evidencias sin conexión | Como conductor, quiero poder capturar y guardar evidencias aunque no tenga señal de internet, para que mis reportes no dependan de la cobertura y tenga un respaldo digital fiable. | 8 |
| 11 | US-28 | Sincronizar registros pendientes | Como conductor, quiero sincronizar automáticamente mis registros pendientes cuando la conectividad se restablece, para conservar la continuidad operativa sin pérdida de datos. | 5 |
| 12 | US-29 | Consultar estado de validación y sincronización | Como conductor, quiero consultar el estado de mis registros (guardados, sincronizados, observados), para saber si mi trabajo quedó correctamente registrado. | 3 |
| 13 | US-11 | Registrar datos manuales de contingencia | Como conductor, quiero ingresar manualmente el kilometraje o combustible cuando la foto no sea válida después de varios intentos, sin detener la operación, siempre que justifique el motivo. | 3 |
| 14 | US-13 | Reportar incidencia operativa | Como conductor, quiero reportar una incidencia (accidente, avería, bloqueo) durante el servicio, para dejar constancia oficial del evento y que el gestor pueda tomar acciones sin tener que llamarme. | 3 |
| 15 | US-14 | Registrar paradas no planificadas | Como conductor, quiero registrar una parada no planificada (control policial, descanso forzoso), para respaldar los retrasos con evidencia de ubicación y hora. | 5 |
| 16 | US-01 | Consultar servicios programados | Como gestor de operaciones, quiero consultar los servicios logísticos programados por período y estado, para supervisar la operación diaria sin depender de reportes manuales. | 5 |
| 17 | US-02 | Asignar conductor y unidad a servicio | Como gestor de operaciones, quiero asignar un conductor y una unidad a un servicio pendiente, para evitar la doble asignación de recursos y los conflictos internos. | 5 |
| 18 | US-07 | Extraer kilometraje automáticamente | Como gestor, quiero que el sistema extraiga automáticamente la lectura del odómetro desde la foto usando OCR local, para resolver discrepancias de kilometraje con evidencia objetiva. | 8 |
| 19 | US-08 | Validar evidencia de entrega | Como gestor, quiero que el sistema valide automáticamente la evidencia de entrega (foto de la guía firmada), para contar con un respaldo verificable que permita responder reclamos de clientes. | 8 |
| 20 | US-09 | Validar comprobante de combustible | Como gestor, quiero validar automáticamente los comprobantes de combustible (foto de la boleta), para detectar inconsistencias como grifos fuera de ruta o fechas incorrectas. | 8 |
| 21 | US-12 | Detectar anomalías operativas | Como gestor, quiero que el sistema detecte automáticamente anomalías (kilometraje inusualmente alto/bajo, consumo fuera de rango), para intervenir oportunamente antes de que se conviertan en un reclamo. | 8 |
| 22 | US-15 | Consultar incidencias y observaciones | Como gestor, quiero consultar el listado de incidencias y observaciones activas, ordenadas por severidad, para priorizar las acciones correctivas. | 5 |
| 23 | US-17 | Consultar trazabilidad completa del servicio | Como gestor, quiero ver una línea de tiempo completa de cada servicio: asignación, inicio, evidencias, incidencias, validaciones y cierre, para auditar la ejecución paso a paso. | 8 |
| 24 | US-18 | Visualizar alertas operativas | Como gestor, quiero ver un panel de alertas activas (anomalías, incidencias no resueltas) con niveles de severidad, para actuar rápidamente. | 5 |
| 25 | US-16 | Resolver incidencia con trazabilidad | Como gestor, quiero cerrar una incidencia registrando la resolución y evidencia de cierre, para mantener un historial auditable. | 3 |
| 26 | US-19 | Consultar indicadores de confiabilidad | Como gestor, quiero consultar indicadores como % de evidencias validadas automáticamente, número de observaciones, y tiempo promedio de revisión, para evaluar la calidad del registro operativo. | 5 |
| 27 | US-20 | Generar reporte de servicios validados | Como gestor, quiero generar un reporte en PDF o Excel de los servicios completados con sus evidencias validadas, para sustentar decisiones ante clientes o la gerencia. | 5 |
| 28 | US-21 | Comparar desempeño por unidad y conductor | Como gestor, quiero comparar métricas (kilometraje promedio, consumo por km, incidencias) entre diferentes unidades y conductores, para identificar patrones de ineficiencia. | 5 |
| 29 | US-05 | Consultar historial de servicios | Como gestor, quiero consultar el historial de servicios ejecutados en un rango de fechas, para revisar el desempeño operativo y detectar patrones de incumplimiento. | 3 |
| 30 | TS-02 | Gestión de servicios vía API | Como Developer, quiero disponer de endpoints RESTful para crear, consultar y actualizar servicios (CRUD), para que la aplicación web y móvil puedan consumirlos. | 5 |
| 31 | TS-03 | Captura y validación de evidencias vía API | Como Developer, quiero un endpoint para subir imágenes de evidencias y otro para consultar el estado de validación, para que la app móvil pueda enviar fotos y verificar su procesamiento. | 8 |
| 32 | TS-04 | Gestión de incidencias y alertas vía API | Como Developer, quiero endpoints para reportar incidencias, consultar alertas y cerrar casos, para integrar la lógica de incidencias con otros sistemas. | 5 |
| 33 | TS-05 | Reportes e indicadores vía API | Como Developer, quiero endpoints que entreguen métricas agregadas y generen reportes en PDF, para que el frontend pueda mostrar dashboards. | 5 |
| 34 | TS-01 | Autenticación y autorización API | Como Developer, quiero implementar autenticación OAuth2 (Auth0) con tokens JWT y control de acceso basado en roles, para proteger los endpoints de la API. | 5 |
| 35 | NF-01 | Recuperación ante fallos de red | Como conductor, quiero que si la sincronización falla, el sistema reintente automáticamente con backoff exponencial (1s, 2s, 4s hasta 3 veces), para no perder datos en rutas con cobertura inestable. | 5 |
| 36 | NF-02 | Seguridad de evidencias en reposo | Como administrador de plataforma, quiero que todas las evidencias almacenadas en el dispositivo móvil estén cifradas con AES-256, para proteger datos sensibles en caso de pérdida o robo del teléfono. | 8 |
